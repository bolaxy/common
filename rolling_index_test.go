...
func TestRollingIndex(t *testing.T) {
	...
	if err == nil || !errors.Is(err, errors.SkippedIndex) {
		t.Fatalf("Should return ErrSkippedIndex")
	}

	_, err = RollingIndex.GetItem(9)
	if err == nil || !errors.Is(err, errors.TooLate) {
	...
	if err == nil || err.Error() != badger.ErrKeyNotFound.Error() { //!errors.Is(err, errors.KeyNotFound) {
...
func TestRollingIndexSkip(t *testing.T) {
	...
	if _, err := RollingIndex.Get(0); err != nil && !errors.Is(err, errors.TooLate) {
