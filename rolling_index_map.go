...
func (rim *RollingIndexMap) AddKey(key uint32) error {
	if _, ok := rim.mapping[key]; ok {
		return errors.NewStoreErr(rim.name, errors.KeyAlreadyExists, fmt.Sprint(key))
	}
...
func (rim *RollingIndexMap) Get(key uint32, skipIndex int) ([]interface{}, error) {
	...
	if !ok {
		return nil, errors.NewStoreErr(rim.name, errors.KeyNotFound, fmt.Sprint(key))
	}
...
func (rim *RollingIndexMap) GetLast(key uint32) (interface{}, error) {
	...
	if len(cached) == 0 {
		return "", errors.NewStoreErr(rim.name, errors.Empty, fmt.Sprint(key))
	}
